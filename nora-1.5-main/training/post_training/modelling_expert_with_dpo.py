...
class VLAWithExpert(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self,vlm_inputs, actions,alpha=10.0,do_dpo=False):
        """
        ...
        if do_dpo:
            action_time_emb_w,action_time_emb_l = action_time_emb.chunk(2,dim=0)

            expert_output_w = self.action_expert(inputs_embeds=action_time_emb_w,
                                    expert_attention_mask=expert_attention_mask.unsqueeze(1).bool(),
                                    position_ids= position_ids,
                                    vlm_key_values=vlm_kv_cache, 
                                    use_cache=True)
             
            expert_output_l = self.action_expert(inputs_embeds=action_time_emb_l,
                                    expert_attention_mask=expert_attention_mask.unsqueeze(1).bool(),
                                    position_ids= position_ids,
                                    vlm_key_values=vlm_kv_cache, 
                                    use_cache=True)
            
            action_out_w = self.action_out_proj(expert_output_w.last_hidden_state.to(torch.float32))
            action_out_l = self.action_out_proj(expert_output_l.last_hidden_state.to(torch.float32))

            u_t_w,u_t_l = u_t.chunk(2,dim=0)

            expert_loss_w = F.mse_loss(action_out_w, u_t_w, reduction='none')
            expert_loss_l = F.mse_loss(action_out_l, u_t_l, reduction='none')

            expert_loss_w = expert_loss_w.mean(dim=list(range(1, len(expert_loss_w.shape))))
            expert_loss_l = expert_loss_l.mean(dim=list(range(1, len(expert_loss_l.shape))))

            model_diff = expert_loss_w - expert_loss_l

            with torch.no_grad():
                
                ref_expert_output_w = self.ref_action_expert(inputs_embeds=action_time_emb_w,
                                        expert_attention_mask=expert_attention_mask.unsqueeze(1).bool(),
                                        position_ids= position_ids,
                                        vlm_key_values=vlm_kv_cache, 
                                        use_cache=True)
                ref_expert_output_l = self.ref_action_expert(inputs_embeds=action_time_emb_l,
                                        expert_attention_mask=expert_attention_mask.unsqueeze(1).bool(),
                                        position_ids= position_ids,
                                        vlm_key_values=vlm_kv_cache, 
                                        use_cache=True)
                ref_action_out_w = self.ref_action_out_proj(ref_expert_output_w.last_hidden_state.to(torch.float32))
                ref_action_out_l = self.ref_action_out_proj(ref_expert_output_l.last_hidden_state.to(torch.float32))

                ref_loss_w = F.mse_loss(
                    ref_action_out_w, u_t_w, reduction="none"
                )
                ref_loss_w = ref_loss_w.mean(dim=list(range(1, len(ref_loss_w.shape))))

                ref_loss_l = F.mse_loss(
                    ref_action_out_l, u_t_l, reduction="none"
                )
                ref_loss_l = ref_loss_l.mean(dim=list(range(1, len(ref_loss_l.shape))))
            ...
            print(inside_term>0/inside_term.size(0))

            dpo_loss = -1 * F.logsigmoid(inside_term).mean() 
            
            #loss = dpo_loss
            loss = dpo_loss 
            return {'implicit_acc':implicit_acc,'dpo_loss': dpo_loss,'combined_loss':loss,'vlm_loss':vlm_outputs.loss}
